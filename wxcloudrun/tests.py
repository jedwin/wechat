...
def print_green(content):
    print_green(f'\033[32m{content}\033[0m')

def print_red(content):
    print_red(f'\033[31m{content}\033[0m')

def print_blue(content):
    print_blue(f'\033[34m{content}\033[0m')
...
class TestModel(TestCase):
    ...
    def test_login(self):
        ...
        print('\033[32m对有权限玩家，游戏开场画面显示正常')
        ...
        print('\033[32m对已分配权限，但游戏未激活时，提示内容显示正常')
        ...
        print('\033[32m对有权限玩家不指定游戏名时，游戏选择画面返回正常')
        ...
        response = views2.game(request)
        self.assertContains(response, self.game.show_opening())
        print('\033[32m对有权限玩家不指定游戏，但打开直接进入游戏开关时，游戏开场内容显示正常')
    ...
    def test_process(self):
        ...
        if result_dict['result']:
            print_green(f'{result_dict["errmsg"]}')
        else:
            print_red(f'{result_dict["errmsg"]}')
        ...
        # print(response.content.decode('utf-8'))
        self.assertContains(response, self.game.show_opening())
        print_green(f'【游戏开场】\033[0m内容显示正常')
        ...
        while num_of_step < TEST_MAX_STEP:
            ...
            if cur_quest.show_next:
                ...
                if len(available_answers_list) == 0:
                    if SHOW_PROCESS: print_red(f'【注意】\033[0m：关卡\033[34m【{cur_quest.quest_trigger}】\033[0m是\033[32m选择题\033[0m，但没有可用选项，无法继续测试。可能是前置条件设置有误或关卡缺失')
                    break
                # 至少有一个可用选项
                ...
                if len(not_available_answers_list) > 0:
                    if SHOW_PROCESS: print_red(f'【注意】\033[0m：本关卡关联了{len(not_available_answers_list)}个缺失关卡 {not_available_answers_list}')
                # 随机选择一个选项作为答案
                answer = available_answers_list[randint(0, num_of_next-1)]
                if SHOW_PROCESS: print(f'随机选择的答案是：\033[43m【{answer}】')
            ...
            try:
                ...
                print_red(f'【注意】\033[0m：下一个关卡\033[31m【{next_quest_trigger}】\033[0m不存在，无法继续测试')
                break
            # 更新奖励列表
            if cur_quest.reward_id > 0 and cur_quest.reward_id not in reward_list:
                reward_list.append(cur_quest.reward_id)
                if SHOW_PROCESS: print(f'正常进入关卡\033[34m【{cur_quest.quest_trigger}】\033[0m，并获得奖励ID：\033[45m【{cur_quest.reward_id}】')
                ...
                if SHOW_PROCESS: print(f'正常进入关卡\033[34m【{cur_quest.quest_trigger}】')
            ...
            if cur_quest.next_list == '':
                print_red(f'【注意】\033[0m：关卡\033[31m【{cur_quest.quest_trigger}】\033[0m的\033[31m下一步为空\033[0m，无法继续测试')
                break
            if SHOW_PROCESS: print(f'目前获得奖励列表：\033[35m{reward_list}')
            ...
            if num_of_step % 4 == 2:
                ...
                if SHOW_PROCESS: print_blue(f'第{num_of_step}步回答错误时，错误提示显示正常')
                error_tips_count += 1
        ...
        if is_cleared:
            ...
            self.assertContains(response, self.game.show_clear_notice())
            self.assertContains(response, 'id="showClearCode"')
            print_green(f'已通关后再次进入游戏时，通关画面显示正常')
            test_result = True
        
        # 判断是否完成了测试步骤
        if content_is_ok: print_green(f'{content_count}次进入关卡，内容显示正常')
        if hint_is_ok: print_green(f'{hint_count}个关卡中配置有提示，内容显示正常')
        if option_is_ok: print_green(f'{option_count}条选择题，选项显示正常')
        if answer_is_ok: print_green(f'{answer_count}条问答题，答题框显示正常')
        if error_tips_is_ok: print_green(f'{error_tips_count}次故意回答错误时，错误提示显示正常')
        if len(reward_list) > 0: 
            print_green(f'获得奖励ID：{reward_list}') 
        else: 
            print_green(f'未获得任何奖励')
        if test_result:
            print_green(f'流程测试完成，顺利达到通关条件')
        elif num_of_step >= TEST_MAX_STEP:
            print_red(f'流程测试未完成\033[0m, 已达到最大测试步数{TEST_MAX_STEP}仍未能通关')
        else:
            print_red(f'流程测试未完成\033[0m, 共进行{num_of_step}步')
    ...
    def test_ExplorerGame_and_Quest(self):
        ...
        ret_dict = handle_player_command(app_en_name=self.app.en_name, game_name=self.game.name, 
                                         open_id=self.user.id, cmd='test', user_name=self.user.username)
        self.assertEqual(ret_dict['error_msg'], f'游戏{self.game.name}未启动或已过活动时间')
        print_green(f'游戏未启用时调用，返回正常')
        self.game.is_active = True
        self.game.save()
        ...
        ret_dict = handle_player_command(app_en_name=self.app.en_name, game_name=self.game.name, 
                                         open_id=self.user.id, cmd='test', user_name=self.user.username)
        self.assertEqual(ret_dict['error_msg'], f'游戏{self.game.name}没有入口任务')
        print_green(f'游戏没有配置入口时调用，返回正常')
        self.game.entry = TEST_GAME_ENTRY
        self.game.save()

        # 游戏入口任务不存在
        self.game.entry = 'wrong_entry'
        self.game.save()
        ret_dict = handle_player_command(app_en_name=self.app.en_name, game_name=self.game.name, 
                                         open_id=self.user.id, cmd='test', user_name=self.user.username)
        self.assertEqual(ret_dict['error_msg'], f'游戏{self.game.name}入口任务{self.game.entry}不存在')
        print_green(f'游戏入口任务不存在时调用，返回正常')

        # 游戏入口任务不存在，且cmd为空，且wait_status为空，应返回所有可进入的关卡
        ret_dict = handle_player_command(app_en_name=self.app.en_name, game_name=self.game.name, 
                                         open_id=self.user.id, user_name=self.user.username)
        self.assertEqual(len(ret_dict['reply_options']), 3)
        print_green(f'游戏入口任务不存在，且cmd为空，且wait_status为空时调用，正常返回所有可进入关卡')
        self.game.entry = TEST_GAME_ENTRY
        self.game.save()
