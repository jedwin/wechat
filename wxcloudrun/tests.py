...
class TestModel(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # import the quests
        # if TEST_SETTINGS_FILE ends with .csv or .json
    ...
    def test_login(self):
        print(f'开始测试\033[31m【系统权限逻辑】\033[0m')
    ...
    def test_process(self):
        ...
        print(f'开始测试游戏\033[31m【{self.game.name}】\033[0m流程')
        # 测试玩家首次进入游戏的返回
        response = views2.game(request)
        # print(response.content.decode('utf-8'))
        self.assertContains(response, self.game.show_opening())
        ...
        while num_of_step < TEST_MAX_STEP:
            ...
            if cur_quest.show_next:
                ...
                if len(available_answers_list) == 0:
                    if SHOW_PROCESS: print(f'【注意】：关卡\033[31m【{cur_quest.quest_trigger}】\033[0m是\033[32m选择题\033[0m，但没有可用选项，无法继续测试。可能是前置条件设置有误或关卡缺失')
                    break
                # 至少有一个可用选项
                if SHOW_PROCESS: print(f'关卡\033[31m【{cur_quest.quest_trigger}】\033[0m是\033[32m选择题\033[0m，{num_of_next}个可选项都显示正常：{available_answers_list}')
                option_count += 1
                if len(not_available_answers_list) > 0:
                    if SHOW_PROCESS: print(f'【注意】：本关卡关联了{len(not_available_answers_list)}个缺失关卡 {not_available_answers_list}')
                # 随机选择一个选项作为答案
                ...
                if SHOW_PROCESS: print(f'关卡\033[31m【{cur_quest.quest_trigger}】\033[0m是\033[34m问答题\033[0m，答案框显示正常。正确答案是：{cur_quest.answer_list}')
                answer_count += 1
            ...
            try:
                ...
                print(f'【注意】：下一个关卡\033[31m【{next_quest_trigger}】\033[0m不存在，无法继续测试')
                break
            # 更新奖励列表
            if cur_quest.reward_id > 0 and cur_quest.reward_id not in reward_list:
                reward_list.append(cur_quest.reward_id)
                if SHOW_PROCESS: print(f'正常进入关卡\033[31m【{cur_quest.quest_trigger}】\033[0m，并获得奖励ID：\033[45m【{cur_quest.reward_id}】\033[0m')
                ...
                if SHOW_PROCESS: print(f'正常进入关卡\033[31m【{cur_quest.quest_trigger}】\033[0m')
            ...
            if cur_quest.next_list == '':
                print(f'【注意】：关卡\033[31m【{cur_quest.quest_trigger}】\033[0m的\033[31m下一步为空\033[0m，无法继续测试')
                break
            ...
            if num_of_step % 4 == 0:
                ...
                if SHOW_PROCESS: print(f'\033[31m第{num_of_step}步回答错误时，错误提示显示正常\033[0m')
                error_tips_count += 1
